...
export function useAddCandidat(userId: string | undefined): UseAddCandidatResult {
  ...
  const addCandidat = useCallback(
    async (candidat: CandidatRecherche, saison: number): Promise<AddCandidatResult> => {
      ...
      // Guard via ref — immunisé contre les stale closures
      ...
      try {
        ...
        // 2. Upsert sans écraser les données existantes (ignoreDuplicates: true)
        ...
        const { error: pariError } = await supabase
          .from("paris")
          .insert({ candidat_id: candidatId, joueur: userId, saison, mort: false });
